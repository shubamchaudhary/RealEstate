      ...
      <li className=" relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]">
        <Link className="contents" to={`/category/${listing.type}/${id}`}>
         <img className="h-[170px] w-full object-cover hover:scale-105 transition-scale duration-200 ease-in"
           loading="lazy"
          src={listing.imgUrls[0]} alt='house img'/>
        ...
        {onEdit && (
               <MdModeEditOutline onClick={() => onEdit(listing.id)} className='absolute bottom-2 right-[32px] text-xl' />
              )}
        {onDelete &&( 
              <MdDelete onClick={() => onDelete(listing.id)} className='absolute bottom-2 right-2 text-xl text-red-500'/>)}
